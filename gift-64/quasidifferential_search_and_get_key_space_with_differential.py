...
def get_quasidifferentials_by_one_difference_route(difference_route, route_number):
    ...
    filename = "result/quasidifferentials_r_{}_with_differential_differential_route_{}.txt".format(difference_characteristic_round, route_number)
    f = open(filename, "a")
    ...
    def get_masterkey_matrix(difference_characteristic, linear_trail_all):
        ...
        for trail_number in range(len(linear_trail_all)):
            ...
            for round in range(0, difference_characteristic_round):
                ...
                mask_c = (1 << (state_bits - 1)) | (((rcs >> 5) & 0x1) << round_constant_positions[5]) | (((rcs >> 4) & 0x1) << round_constant_positions[4]) | (((rcs >> 3) & 0x1) << round_constant_positions[3]) | (((rcs >> 2) & 0x1) << round_constant_positions[2]) | (((rcs >> 1) & 0x1) << round_constant_positions[1]) | (((rcs >> 0) & 0x1) << round_constant_positions[0])
            ...
            if len(correlation_key) > 0:
                ...
                if ((-1) ** correlation_negative_1_number) < 0:
                    ...
                    key_str += " = 0"
                one_trail_key_str = "equation: " + key_str
            else:
                one_trail_key_str = "equation: 1 = 1 (mask=0)"
...
from res import r_13_differential as differentials
difference_characteristics = differentials.routes[0]
